...
describe('isAllowedLinkUrl', () => {
  it('許可スキームと相対パス・data:image を許可する', () => {
    for (const url of ['https://x', 'http://x', 'mailto:a@b', 'file:///c/x.txt', 'sub/x.png', './x', '../x', '#a', 'data:image/png;base64,AAAA']) {
      expect(isAllowedLinkUrl(url), url).toBe(true);
    }
  });

  it('実行系・許可外スキームを拒否する(空白/制御文字/実体参照/パーセントエンコード偽装込み)', () => {
    for (const url of ['javascript:alert(1)', 'JaVaScRiPt:alert(1)', 'java\tscript:alert(1)', 'java\nscript:alert(1)', 'java%09script:alert(1)', 'vbscript:x', 'data:text/html;base64,AAAA', 'tel:+81', 'ftp://x/y']) {
