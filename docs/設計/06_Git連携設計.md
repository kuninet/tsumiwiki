...
| 機能 | 実装 |
|---|---|
| 履歴一覧 | `git log --format=... -- <path>`(#66 でリネーム追跡なし。理由は表下参照) |
...
- 履歴一覧に `--follow` を付けない理由(#66): git の `--follow` は独自のリネーム検出パスを使い `-M` 閾値が効かず類似度 50% の固定判定になる。TsumiWiki ではテンプレ由来など初期内容が類似した別文書のコミットが誤って混入する症状が確認された。代償としてリネーム後の文書はリネーム前の履歴が途切れる(現在のリネーム機能はあまり使われない前提での判断)。
